...
import com.nik.diceassignment.DrawableMatcher.withDrawable
...
open class BaseTest {
    ...
    fun testState(textActionResource: Int, drawableResource: Int) {
        onView(withId(R.id.lemonade_text))
            .check(matches(ViewMatchers.withText(textActionResource)))
        onView(withId(R.id.lemonade_image)).check(
    ...
    fun juiceLemon() {
        while (onView(withDrawable(R.drawable.lemon_squeeze)).isPresent()) {
            onView(withId(R.id.lemonade_image)).perform(click())
        }
    }
